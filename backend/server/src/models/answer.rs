...
use crate::models::question::{MultiOptionData, MultiOptionQuestionOption, QuestionData, QuestionType, QuestionTypeParent};
...
impl Answer {
    pub async fn create(user_id: i64, application_id: i64, question_id: i64, answer_data: AnswerData, mut snowflake_generator: SnowflakeIdGenerator, transaction: &mut Transaction<'_, Postgres>) -> Result<i64, ChaosError> {
    ...
    pub async fn get(id: i64, transaction: &mut Transaction<'_, Postgres>) -> Result<Answer, ChaosError> {
        ...
        let answer_data = AnswerData::from_answer_raw_data(answer_raw_data.question_type, answer_raw_data.short_answer_answer, answer_raw_data.multi_option_answers, answer_raw_data.ranking_answers);
    ...
    pub async fn get_all_common_by_application(application_id: i64, transaction: &mut Transaction<'_, Postgres>) -> Result<Vec<Answer>, ChaosError> {
        ...
        let answers = answer_raw_data.into_iter().map(|answer_raw_data| {
            let answer_data = AnswerData::from_answer_raw_data(answer_raw_data.question_type, answer_raw_data.short_answer_answer, answer_raw_data.multi_option_answers, answer_raw_data.ranking_answers);
    ...
    pub async fn get_all_by_application_and_role(application_id: i64, role_id: i64, transaction: &mut Transaction<'_, Postgres>) -> Result<Vec<Answer>, ChaosError> {
        let answer_raw_data: Vec<AnswerRawData> = sqlx::query_as(
            "
                SELECT
                    a.id,
    ...
    pub async fn update(id: i64, answer_data: AnswerData, transaction: &mut Transaction<'_, Postgres>) -> Result<(), ChaosError> {
        answer_data.validate()?;
        ...
        sqlx::query!("UPDATE applications SET updated_at = $1 WHERE id = $2", Utc::now(), answer.application_id)
            .execute(transaction.deref_mut())
            .await?;
...
impl AnswerData {
    ...
    fn from_answer_raw_data(question_type: QuestionType, short_answer_answer: Option<String>, multi_option_answers: Option<Vec<i64>>, ranking_answers: Option<Vec<i64>>) -> Self {
        ...
        {
            let options = multi_option_answers.expect("Data should exist for MultiOptionData variants");
    ...
    pub async fn insert_into_db(self, answer_id: i64, transaction: &mut Transaction<'_, Postgres>) -> Result<(), ChaosError> {
        match self {
    ...
    pub async fn delete_from_db(self, answer_id: i64, transaction: &mut Transaction<'_, Postgres>) -> Result<(), ChaosError> {
        match self {
            Self::ShortAnswer(_) => {
                sqlx::query!("DELETE FROM short_answer_answers WHERE answer_id = $1", answer_id)
                    .execute(transaction.deref_mut())
                    .await?;
            },
            Self::MultiChoice(_)
            | Self::MultiSelect(_)
            | Self::DropDown(_) => {
                sqlx::query!("DELETE FROM multi_option_answer_options WHERE answer_id = $1", answer_id)
                    .execute(transaction.deref_mut())
                    .await?;
            },
            Self::Ranking(_) => {
                sqlx::query!("DELETE FROM ranking_answer_rankings WHERE answer_id = $1", answer_id)
                    .execute(transaction.deref_mut())
                    .await?;
            }
        }
