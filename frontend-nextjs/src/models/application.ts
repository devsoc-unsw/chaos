...
export async function createOrGetApplication(
  ...
  return await apiRequest<{ application_id: string }>(
    `/api/v1/campaign/${campaignId}/apply`,
    {
      method: "POST",
    },
  );
...
export async function getApplication(
  ...
  return await apiRequest<ApplicationDetails>(
    `/api/v1/application/${applicationId}`,
  );
}

export async function getInProgressApplication(
  applicationId: string,
): Promise<ApplicationDetails> {
  return await apiRequest<ApplicationDetails>(
...
export async function updateApplicationRating(
  ...
  return await apiRequest<AppMessage>(
    `/api/v1/application/${applicationId}/rating`,
    {
      method: "PUT",
      body: {
        rating,
...
export async function getApplicationRatingsSummary(
  campaignId: string,
): Promise<ApplicationRatingSummary[]> {
  return await apiRequest<ApplicationRatingSummary[]>(
    `/api/v1/campaign/${campaignId}/avg_ratings`,
  );
}

export async function updateApplicationPrivateStatus(
  applicationId: string,
  status: ApplicationStatus,
): Promise<AppMessage> {
  return await apiRequest<AppMessage>(
    `/api/v1/application/${applicationId}/private`,
    {
      method: "PATCH",
      body: status,
    },
